      ...
      <div class="col-lg-8">
        <div class="left-area cart">
              ...
              @include('includes.form-success')
                <thead class="cart__row cart__header small--hide">
                  ...
                  <tbody>
                    ...
                    <tr class="cremove{{ $product['item']['id'].$product['size'].$product['color'].str_replace(str_split(' ,'),'',$product['values']) }} cart__row border-bottom line1 cart-flex border-top">

                      <td  class="cart-delete text-center small--hide">
                        <span data-class="cremove{{ $product['item']['id'].$product['size'].$product['color'].str_replace(str_split(' ,'),'',$product['values']) }}" data-href="{{ route('product.cart.remove',$product['item']['id'].$product['size'].$product['color'].str_replace(str_split(' ,'),'',$product['values'])) }}" class="removecart cart-remove btn btn--secondary cart__remove remove-icon position-static" data-bs-toggle="tooltip" data-bs-placement="top" title="Remove item"><i class="icon an an-times-r"></i></span>
                      </td>

                      <td  class="cart__image-wrapper cart-flex-item">
                        <a href="{{ route('front.product', $product['item']['slug']) }}"><img class="cart__image blur-up lazyload" data-src="{{ $product['item']['photo'] ? asset('assets/images/products/'.$product['item']['photo']):asset('assets/images/noimage.png') }}" src="{{ $product['item']['photo'] ? asset('assets/images/products/'.$product['item']['photo']):asset('assets/images/noimage.png') }}" alt="" width="80" /></a>
                      ...
                      <td class="cart__meta small--text-left cart-flex-item">
                                                <div class="list-view-item__title">
                                                    <a href="product-layout1.html">{{mb_strlen($product['item']['name'],'utf-8') > 800 ? mb_substr($product['item']['name'],0,800,'utf-8').'...' : $product['item']['name']}}</a>
                                                </div>
                                                ...
                                                <div class="cart__meta-text">
                                                   @if(!empty($product['color']))
                                                    Color: <span id="color-bar" style="border: 10px solid #{{$product['color'] == "" ? "white" : $product['color']}};"></span> @endif<br>
                                                    @if(!empty($product['size']))
                                                    Size: {{ $product['item']['measure'] }}{{str_replace('-',' ',$product['size'])}}
                                                    @endif
                                                ...
                                                <div class="cart-price d-md-none">
                                                    <span class="money fw-500">{{ App\Models\Product::convertPrice($product['item_price']) }}</span>
                                                </div>
                                            </td>
          ...
          @if($product['item']['type'] == 'Physical')
              ...
              <input type="hidden" class="size_price" value="{{$product['size_price']}}">   
                                  ...
                                  <span class="qttotal1 cart__qty-input qty" id="qty{{$product['item']['id'].$product['size'].$product['color'].str_replace(str_split(' ,'),'',$product['values'])}}">{{ $product['qty'] }}</span>
        ...
        @endif
                      ...
                      <td class="cart-price cart-flex-item text-center small--hide">
                        <span id="prc{{$product['item']['id'].$product['size'].$product['color'].str_replace(str_split(' ,'),'',$product['values'])}}"  class="money fw-500">
          ...
          <div class="order-box cart_info">
            ...
            <div class="total-price">
              ...
              <form id="coupon-form" class="coupon">
                ...
                <div class="form-group mb-0">
                                            ...
                                            <div class="input-group flex-nowrap">
                                              <input type="text" placeholder="{{ $langg->lang133 }}" id="code" required="" autocomplete="off" class="input-group__field">
                <input type="hidden" class="coupon-total" id="grandtotal" value="{{ Session::has('cart') ? App\Models\Product::convertPrice($mainTotal) : '0.00' }}">
                                                <span class="input-group__btn">
                                                   <button type="submit" id="codebtnapply" class="btn rounded-end text-nowrap">{{ $langg->lang134 }}</button>
                                                </span>
                                            </div>
                                        </div>
              ...
              <div class="coupon-code-wrap pb-4">
                  ...
                  @if($mincart <= $totalQty)
                    @if((preg_replace('/\D/', '', (Session::has('cart') ? App\Models\Product::convertPrice($mainTotal) : '0.00'))) >= $minvalue)
