...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Set Thumbnail
        $img = Image::make(public_path().'/assets/images/products/'.$prod->photo);
        $thumbnail = Str::random(10).'.jpg';
        $img->save(public_path().'/assets/images/thumbnails/'.$thumbnail);
            ...
            if (!Product::where('sku',$line[0])->exists()){
                ...
                $thumb_url = '';
                    ...
                    if (strpos($contentType, 'image/') !== false) {
                        $fimg = Image::make($line[5]);
                        $fphoto = Str::random(10).'.jpg';
                        $fimg->save(public_path().'/assets/images/products/'.$fphoto);
                        $input['photo']  = $fphoto;
                        $thumb_url = $line[5];
                    }else{
                        $fimg = Image::make(public_path().'/assets/images/noimage.png'); 
                        $fphoto = Str::random(10).'.jpg';
                        $fimg->save(public_path().'/assets/images/products/'.$fphoto);
                        $input['photo']  = $fphoto;
